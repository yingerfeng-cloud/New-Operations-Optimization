...
export interface FormulaDef { formula_id: string; name: string; kind: 'constraint' | 'objective'; display_formula: string; dsl_formula: string; tokens: FormulaToken[]; foreach: string[]; referenced_sets: string[]; referenced_parameters: string[]; referenced_variables: string[]; free_indices: string[]; big_m?: boolean; compile_status: 'ready' | 'error' | 'unsupported'; compile_error?: string }
