...
- legacy 平台：`http://localhost:8000/legacy`
- legacy 原路径：`http://localhost:8000/prototype.html`

`prototype.html` 和 `agent_console.html` 仅用于兼容或回归验证。日常模型、组件、任务和结果操作应使用 React 前端。
...
打开 `http://localhost:8000/`。FastAPI 会托管 `frontend/dist`，并保留 `/api` 与 legacy 路径。
...
使用 `/legacy` 或 `/prototype.html` 访问；资源由 `/static` 提供，不建议使用 `file://`。
...
- legacy 页面继续保留，但正式功能优先在 React 前端实现。
