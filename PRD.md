...
Skill 是模型服务化后的平台一等能力，承接模型发布后的接口契约、Schema、启停、在线调试、Agent 绑定和调用审计。Agent 是 Skill 编排入口，负责自然语言意图识别、参数收集、默认值/示例值确认、人工确认调用和结果解释，不直接替代人工调度决策。

生产安全边界：所有优化结果均为辅助决策建议，不构成自动控制指令；高风险调用必须保留人工确认与复核。LLM Key 不进入前端响应和普通 runtime JSON。
...
- NLP 已实现 Ipopt 真实求解接入，不再作为纯预留能力描述。
- NLP 适用于连续变量非线性模型，例如 `nonlinear_hydro_power_demo` 中的 `power = k * flow * head`。
- Ipopt 结果通常为局部最优或求解器终止状态，不承诺全局最优。
- 含整数变量和非线性表达式的模型归为 `MINLP_RESERVED`，当前不作为生产级求解能力默认开放。
- 对可线性化非线性关系，平台推荐使用 1D/2D PWL 或 McCormick 转换为 LP/MILP 后交由 HiGHS 求解。
